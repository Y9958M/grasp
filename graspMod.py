...
class GraspBlInvbraHdr(Base):
    ...
    fromid = mapped_column(TINYINT(4), nullable=False, server_default=text("'0'"), comment='单据来源 0初始 1手工 2转单 3导入')
    ...
    inv_date = mapped_column(DateTime, nullable=False, comment='开票日期')
...
class GraspBlInvsupDtl(Base):
    ...
    remark = mapped_column(String(255), comment='备注')
...
class GraspBlInvsupHdr(Base):
    ...
    bill_code = mapped_column(String(64), nullable=False, server_default=text("''"), comment='客户单号')
    ...
    supplier_name = mapped_column(String(64), nullable=False, server_default=text("''"), comment='供应商名称')
    buyer_no = mapped_column(INTEGER(11), nullable=False, server_default=text("'0'"), comment='购买方ID')
    buyer_name = mapped_column(String(64), nullable=False, server_default=text("''"), comment='购买方名称')
    ...
    fromid = mapped_column(TINYINT(4), nullable=False, server_default=text("'0'"), comment='单据来源 0初始 1手工 2转单 3导入')
    inv_id = mapped_column(String(36), nullable=False, server_default=text("''"), comment='发票代码')
    inv_no = mapped_column(String(36), nullable=False, server_default=text("''"), comment='发票号码')
    inv_name = mapped_column(String(64), nullable=False, server_default=text("''"), comment='发票名称')
    inv_date = mapped_column(DateTime, nullable=False, comment='开票日期')
    check_id = mapped_column(String(36), nullable=False, comment='校验码')
    machine_no = mapped_column(BIGINT(20), nullable=False, comment='机器编号')
    taxpayer_id = mapped_column(String(36), nullable=False, server_default=text("''"), comment='纳税人识别号')
    fi_addr = mapped_column(String(64), nullable=False, server_default=text("''"), comment='地址')
    fi_tel = mapped_column(String(64), nullable=False, server_default=text("''"), comment='地址和电话')
    accounts_bank = mapped_column(String(64), nullable=False, server_default=text("''"), comment='开户行')
    accounts_code = mapped_column(String(64), nullable=False, server_default=text("''"), comment='开户帐号')
    userid = mapped_column(BIGINT(18), nullable=False, server_default=text("'0'"), comment='用户ID')
...
class GraspOnhand(Base):
    ...
    id = mapped_column(BIGINT(20), primary_key=True, comment='序号')
    sid = mapped_column(INTEGER(1), nullable=False, server_default=text("'0'"), comment='数据标识 0不生效 1生产 2测试 3 作废')
    storage_no = mapped_column(INTEGER(11), nullable=False, comment='收货仓')
    pro_no = mapped_column(BIGINT(18), nullable=False, comment='商品编码')
    pro_name = mapped_column(String(255), nullable=False, comment='商品名称')
    qty_aval = mapped_column(DECIMAL(18, 3), nullable=False, comment='可用数量')
    price_pur_last = mapped_column(DECIMAL(18, 6), nullable=False, comment='最近采购单价')
    ldt = mapped_column(DateTime, nullable=False, server_default=text('CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP'), comment='最后更新时间')
    spec = mapped_column(String(255), comment='规格型号')
    units = mapped_column(String(255), comment='单位')
    remark = mapped_column(String(255), comment='备注')
...
class GraspSupplier(Base):
    ...
    addr = mapped_column(String(36), nullable=False, server_default=text("''"), comment='地址')
    tel = mapped_column(String(36), nullable=False, server_default=text("''"), comment='电话')
    social_credit_code = mapped_column(String(36), nullable=False, server_default=text("''"), comment='统一信用码')
    tax_code = mapped_column(String(36), nullable=False, server_default=text("''"), comment='税号')
    accounts_bank = mapped_column(String(36), nullable=False, server_default=text("''"), comment='银行')
